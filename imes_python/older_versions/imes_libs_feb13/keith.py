...
def print_iv_biases(keith_dict):
    ...
    iv_biases, _ = get_bias_voltages(float(keith_dict['max_bias'].text()),
                                     int(keith_dict['voltage_steps'].text()))
...
def print_cv_biases(keith_dict):
    ...
    _, cv_biases = get_bias_voltages(float(keith_dict['max_bias'].text()),
                                     int(keith_dict['voltage_steps'].text()))
...
def get_current_continuously(keith_dict, df, df_i):
    ...
    if keith_dict['measure_current_now'].isChecked():  # if current started
        ...
        bias = float(keith_dict['set_bias'].text())
        ...
        if df_i % 3 == 0:  # plot current over time
            plt.ion
...
def measure_cv(keith_dict, df, df_i):
    ...
    capacitance = 8.33
    df['cv_capacitance'].iloc[df_i] = str(capacitance)

    view_cv_data(keith_dict)
    plt.pause(2)
    plt.close()
...
def keith_rh_seq(keith_dict):
    # measure keithley functions during RH sequence
    keith_dict['keith_busy'] = True
    if keith_dict['iv_rh_seq'].isChecked():
        measure_iv(keith_dict)
        time.sleep(2)
    if keith_dict['cv_rh_seq'].isChecked():
        measure_cv(keith_dict)
        time.sleep(2)
    if keith_dict['bs_rh_seq'].isChecked():
        measure_bias_seq(keith_dict)
        time.sleep(2)
        time.sleep(float(keith_dict['pause_after_cycle'].text())*60)
    keith_dict['keith_busy'] = False
