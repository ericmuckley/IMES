...
# custom modules
from imes_libs_threaded import ops
from imes_libs_threaded import keith
from imes_libs_threaded import rhmeter
from imes_libs_threaded import sark
from imes_libs_threaded import rh200
...
class Worker(QRunnable):
    # class for threaded worker task
    def __init__(self, task, seq_dict):
    ...
    def run(self):
        ...
        self.seq_dict['output_box'].append('worked')
        time.sleep(2)
        self.seq_dict['output_box'].append('worked AGAIN')
...
class App(QMainWindow):  # create the main window
    ...
    qt_ui_file = 'C:\\Users\\a6q\\imes_python\\IMES_layout_feb_7.ui'
    ...
    def __init__(self):  # initialize application
        ...
        self.pool = QThreadPool()  # set up multi-threading
        ...
        self.ui.run_rh_seq.triggered.connect(self.run_rh_seq0)
        ...
        self.ui.dummy_button.clicked.connect(self.start)
...
# %% ----------- system control functions ------------------------------
    ...
    def process_result(self, task):
        ...
        self.ui.output_box.append('receiving %i' % task)

    def start(self):
        for task in range(10):
            worker = Worker(task, self.seq_dict)
    ...
    def main_loop(self):
        ...
        if self.file_save_dir_is_set:
            ...
            print(self.keith_dict['keithley_busy'])

            # auto scroll to bottom of output box
...
# %%  ---- functions for electrical characterization using Keithley 2420 -----
    ...
    def measure_iv(self):
        ...
        Thread(target=keith.measure_iv, args=(self.keith_dict,)).start()

    def measure_cv(self):
        # Measure C-V curve using Keithley multimeter.
        # keith.measure_cv(self.keith_dict)
        Thread(target=keith.measure_cv, args=(self.keith_dict,)).start()
    ...
    def measure_bias_seq(self):
        ...
        Thread(target=keith.measure_bias_seq, args=(self.keith_dict,)).start()
...
# %% ---------- functions for RH control and sequence ------------------
    ...
    def run_rh_seq(self):
        # Run the RH sequence
        rh_seq_thread = rh200.RhSeqThread(self.seq_dict)  # instantiate thread
        # uncomment next line to run different function at end of thread
        #rh_seq_thread.signals.result.connect(self.rh_seq_done)
        self.pool.start(rh_seq_thread)  # start thread
        #self.pool.waitForDone()

#    def rh_seq_done(self):
#        self.output_box.append('RH sequence complete!!!!!!.')
