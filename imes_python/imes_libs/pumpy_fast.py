...
class Chain(serial.Serial):
    ...
    def __init__(self, port):
        serial.Serial.__init__(self,port=port, baudrate=19200, stopbits=serial.STOPBITS_TWO, parity=serial.PARITY_NONE, timeout=2)
        self.flushOutput()
        self.flushInput()
...
class Pump:
    ...
    """
    def __init__(self, chain, address=0, name='Syringe pump'):
        self.name = name
    ...
    def write(self,command):
        print ("command send: " + self.address + command)
        cmd = self.address + command + '\r'
        self.serialcon.write(cmd.encode())

    def read(self,bytes=5):
        resp = self.serialcon.read(bytes)
    ...
    def setflowrate(self, flowrate):
        ...
        # fast version, withou ack from the pump
        '''
        resp = self.read(5)
        if (resp[-1] == ':' or resp[-1] == '<' or resp[-1] == '>'):
    ...
    def infuse(self):
        ...
        while resp[-1] != '>':
            if resp[-1] == '<': # wrong direction
                ...
                raise PumpError('%s: unknown response to infuse' % self.name)
...
class PHD2000(Pump):
    ...
    def stop(self):
        ...
        if resp[-1] == '*':
            ...
            logging.info('%s: unwxpected response  to stop',self.name)
            #raise PumpError('%s: unexpected response to stop', self.name)
        '''
